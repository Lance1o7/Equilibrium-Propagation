...
if args.task == "MNIST":
    ...
    if args.data_aug:
        transform_train = torchvision.transforms.Compose([torchvision.transforms.RandomHorizontalFlip(0.5), torchvision.transforms.RandomCrop(size=[32, 32], padding=4, padding_mode="edge"), torchvision.transforms.ToTensor(), torchvision.transforms.Normalize(mean=(0.4914, 0.4822, 0.4465), std=(3 * 0.2023, 3 * 0.1994, 3 * 0.2010))])
    else:
        transform_train = torchvision.transforms.Compose([torchvision.transforms.ToTensor(), torchvision.transforms.Normalize(mean=(0.4914, 0.4822, 0.4465), std=(3 * 0.2023, 3 * 0.1994, 3 * 0.2010))])
...
if args.load_path == "":
    if args.model == "MLP":
        ...
        if args.task == "MNIST":
            ...
            if args.model == "CNN":
                model = P_CNN(28, channels, args.kernels, args.strides, args.fc, pools, args.paddings, activation=activation, softmax=args.softmax)
            elif args.model == "VFCNN":
                model = VF_CNN(28, channels, args.kernels, args.strides, args.fc, pools, args.paddings, activation=activation, softmax=args.softmax, same_update=args.same_update)
            ...
            pools = make_pools(args.pools)
            channels = [3] + args.channels
            model = P_CNN(224, channels, args.kernels, args.strides, args.fc, pools, args.paddings, activation=activation, softmax=args.softmax)
...
if args.todo == "train":
    ...
    train(model, optimizer, train_loader, test_loader, args.T1, args.T2, betas, device, args.epochs, criterion, alg=args.alg, random_sign=args.random_sign, check_thm=args.check_thm, save=args.save, path=path, checkpoint=checkpoint, thirdphase=args.thirdphase, scheduler=scheduler, cep_debug=args.cep_debug)
