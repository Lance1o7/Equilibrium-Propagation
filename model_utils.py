    ...
    def __init__(self, in_size, channels, kernels, strides, fc, pools, paddings, activation=hard_sigmoid, softmax=False, same_update=False):
        ...
        for idx in range(len(channels) - 1):
            ...
            if idx > 0:  # backward synapses except for first layer
                self.B_syn.append(torch.nn.Conv2d(channels[idx], channels[idx + 1], kernels[idx], stride=strides[idx], padding=paddings[idx], bias=False))
...
def train(model, optimizer, train_loader, test_loader, T1, T2, betas, device, epochs, criterion, alg="EP", random_sign=False, save=False, check_thm=False, path="", checkpoint=None, thirdphase=False, scheduler=None, cep_debug=False):
    ...
    for epoch in range(epochs):
        ...
        for idx, (x, y) in enumerate(train_loader):
            ...
            if (idx % (iter_per_epochs // 10) == 0) or (idx == iter_per_epochs - 1):
                ...
                print("Epoch :", round(epoch_sofar + epoch + (idx / iter_per_epochs), 2), "\tRun train acc :", round(run_acc, 3), "\t(" + str(run_correct) + "/" + str(run_total) + ")\t", timeSince(start, ((idx + 1) + epoch * iter_per_epochs) / (epochs * iter_per_epochs)))
        ...
        if save:
            ...
            if test_correct > best:
                best = test_correct
                save_dic = {"model_state_dict": model.state_dict(), "opt": optimizer.state_dict(), "train_acc": train_acc, "test_acc": test_acc, "best": best, "epoch": epoch_sofar + epoch + 1}
    ...
    if save:
        save_dic = {"model_state_dict": model.state_dict(), "opt": optimizer.state_dict(), "train_acc": train_acc, "test_acc": test_acc, "best": best, "epoch": epochs}
        save_dic["angles"] = angles
        save_dic["scheduler"] = scheduler.state_dict() if scheduler is not None else None
